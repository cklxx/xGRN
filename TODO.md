...
- [~] A1-full: fused `rms_norm + q_proj` Metal kernel.
    ...
    - [x] Re-bench with `--no-compile-visual-pass` to isolate kernel overhead from compile-state effects. Findings: (a) `--no-compile-visual-pass` alone makes decode **+75 % slower** (1.06 → 1.85 s) — the same compile-state-affects-decode anomaly every qkv-fusion experiment hits; (b) **uncompiled + prim is +3.08 % GRN vs uncompiled-only baseline**, which is the actual irreducible per-call axpby cost; (c) uncompiled + prim's decode is *24 % faster* than uncompiled-only, the opposite of compiled+prim's +45 % decode regression — the decode penalty is a prim×compile interaction, not inherent to prim. So the integration tax is split between (~3 % real kernel overhead) and (compile-state ripple effects we don't control). Net: prim+uncompiled is still +13 % e2e vs production default because dropping compile alone costs ~10 %. Custom primitives plus `mx.compile` over the visual pass are not coexisting well on this MLX version.
