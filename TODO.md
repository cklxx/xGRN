...
- [ ] A1-full: fused `rmsnorm + qkv_proj + rope` Metal kernel with simdgroup matmul. Higher ceiling (~30–40 ms/step expected) but requires writing a tuned bf16 matmul that beats MLX's GEMM. After A3.
- [ ] A3: fused sampling + mask update kernel with `atomic_outputs=True`. Save 5–10 dispatches per step. Per-step impact (5–10 saved per step × 50 steps = 250–500 dispatches per generation) is in the middle between A1-lite (16,800 saved per generation) and A2 (1,400 saved per generation), so the expected wall impact should sit between -0.28% and -1.63%.
