...
- [~] A1-full: fused `rms_norm + q_proj` Metal kernel.
    ...
    - [~] Custom Primitive subclass wired in `xgrn_ext/cpp/simdgroup_matmul_primitive.{h,cpp}` as `SimdgroupMatmul : public mlx::core::Primitive`. mx.compile now sees a NAMED primitive (shape inference works, `array.shape == (M, N)`, `array.dtype == float32`). **`eval_gpu` is blocked on installing the Metal Toolchain** (`xcodebuild -downloadComponent MetalToolchain`). Attempted in-eval_gpu dispatch via `mx::fast::metal_kernel(...)(...)` or `mlx::core::matmul` both crash with `_MTLCommandBuffer addCompletedHandler` assertion because forcing `mx::eval` inside eval_gpu creates a nested command-buffer lifecycle the Metal runtime rejects. The correct axpby-style implementation (allocate output buffer + load precompiled `.metallib` + dispatch via `metal::get_command_encoder`) requires the toolchain.
    - Subclass `mx::fast::Custom` was also tried but **`Custom`'s vtable / vjp / vmap symbols are not exported by libmlx.dylib** (linker error: "vtable for mlx::core::fast::Custom" unresolved). Out-of-tree subclasses of `Custom` are not supported by the MLX install we have; only direct `Primitive` subclasses work.
    - To unblock: `sudo xcodebuild -downloadComponent MetalToolchain`, then add `cpp/xgrn_ext.metal` + `mlx_build_metallib` in CMakeLists + implement eval_gpu axpby-style. See `xgrn_ext/README.md` "Custom Primitive subclass — wired, eval_gpu blocked on Metal Toolchain" for the exact steps.
