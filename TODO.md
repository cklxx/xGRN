...
- [~] A1-full: fused `rms_norm + q_proj` Metal kernel **POC landed** as `fused_norm_qproj` in `xgrn_mlx/grn.py` with parity (fp32 max-abs-diff 2.86e-6) and naive scalar matmul. **Microbench: 9.4 ms / call vs MLX's 1.2 ms = 8× slower** — naive matmul cannot beat Apple's GEMM. Not wired into the block. To finish A1-full, rewrite the matmul inner loop with `simdgroup_matrix_storage` 8×8×8 `simdgroup_multiply` ops, tile along K dimension in shared memory, and tune occupancy for the 20-core M4 Pro. That is a multi-day project tracked separately; the POC kernel is left as scaffolding.
