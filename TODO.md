...
- [~] A1-full: fused `rms_norm + q_proj` Metal kernel.
    ...
    - [ ] Next: subclass `mx::fast::Custom` in `xgrn_ext/cpp/` (instead of just calling `mx::fast::metal_kernel`). Provide a `fallback_` MLX op chain (`a @ b`) for `mx.compile` to trace; override `eval_gpu` to dispatch the kernel. This makes the kernel a named primitive on par with `RMSNorm`/`RoPE`. See `xgrn_ext/README.md` "Next iteration: Custom Primitive subclass" for the concrete steps.
