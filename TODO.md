...
- [~] A1-full: fused `rms_norm + q_proj` Metal kernel.
    ...
    - [ ] Next-iteration paths (deferred): Track A1-full integration is genuinely saturated through both `mx.fast.metal_kernel` AND MLX-only manual fusion on this stack. Real wins now need either (a) MLX C++ extensions to make the kernel a first-class primitive in the compile graph, or (b) a parallel approach: profile what's actually slow inside the 1.55 s/step GRN refine and find new (non-qkv) targets.
