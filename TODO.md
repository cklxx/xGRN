...
- [~] A1-full: fused `rms_norm + q_proj` Metal kernel.
    ...
    - [ ] Next-iteration paths (deferred): (a) move the simdgroup_matmul kernel *behind* `mx.compile` by reimplementing it as an MLX op so the compiler can fuse across it; (b) write the kernel as an MLX C++ extension (`mlx/extensions`) instead of `mx.fast.metal_kernel` — extensions get treated as first-class primitives; (c) skip the visual-pass compile altogether and use the kernel in an uncompiled qkv() — would regress without the compile, but the kernel might compose with future Track B (shapeless whole-stack compile).
