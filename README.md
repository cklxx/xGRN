...
| `xgrn_ext.simdgroup_matmul_primitive` (Custom Primitive subclass attempt) | `SimdgroupMatmul : public mlx::core::Primitive` wired into the extension. Shape inference works (mx.compile now sees a NAMED primitive, `array.shape == (M, N)`, `array.dtype == fp32`). **`eval_gpu` is blocked on the Metal Toolchain xcodebuild component**, which is not installed on this Mac. Two attempted shortcuts both crash with `_MTLCommandBuffer addCompletedHandler` Metal assertion: calling `mx::fast::metal_kernel(...)(...)` from inside eval_gpu or calling `mlx::core::matmul` from inside eval_gpu — forcing `mx::eval` inside eval_gpu creates a nested command-buffer lifecycle the Metal runtime rejects. Also tried subclassing `mx::fast::Custom` (the right superclass conceptually) but its vtable / vjp / vmap symbols are local in `libmlx.dylib` so out-of-tree subclasses can't link. To unblock: `sudo xcodebuild -downloadComponent MetalToolchain`, then add `cpp/xgrn_ext.metal` + `mlx_build_metallib` in `CMakeLists.txt` + implement eval_gpu axpby-style. See `xgrn_ext/README.md` for the exact steps. The raw `simdgroup_matmul_bf16` / `simdgroup_matmul_fp32` C++ functions still work and are what `--fuse-qkv-ext` uses today. |
