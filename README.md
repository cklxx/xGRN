...
| `--fuse-mlp-gate-up` (existing flag, re-bench after profile) | Per-op profile (`outputs/track-a/profile/block_ops.py`) showed MLP matmuls are 56 % of per-step time (3× larger than QKV at 22 %). The existing `--fuse-mlp-gate-up` flag uses MLX matmul on the concatenated gate+up weight per block. **t2i-correct: also regresses** with the same pattern: GRN +5.17 %, end-to-end +8.67 %, decode +8.54 %, CLIP **bit-identical 0.9904** (no numerical change — the mx.compile graph just runs slower with the manual concat). Confirms the diagnosis from `--fuse-qkv-concat`: matmul fusion via Python-level weight concatenation is structurally counter-productive on this mx.compile'd path. |
| `xgrn_ext` MLX C++ extension (Track A1-full real foundation) | New top-level `xgrn_ext/` package: the `simdgroup_matmul` kernel built as an MLX C++ extension (CMake + nanobind, `NB_DOMAIN mlx` for shared array type-casters). Build works on stock macOS+Xcode without the Metal Toolchain because `mx::fast::metal_kernel` compiles MSL at runtime. Isolated microbench at M=544, K=2304, N=2304 (bf16): **xgrn_ext 1.01× of MLX bf16 matmul** (within noise after kernel cache fix). The integration result via `--fuse-qkv-ext`: t2i-correct GRN `75.56 → 78.41 s` (+3.77 %), end-to-end +4.09 %, **decode +15.23 %** (down from `--fuse-qkv-metal`'s +88 %, much closer to noise), CLIP 0.9904 → 0.9206. Better than every Python-level fusion attempt but still net negative. Remaining gap is the `mx.pad` to M%32 dispatch (1400 calls/gen) and `mx::fast::metal_kernel`'s opaque-op status in the compile graph. **Build infrastructure now in place**. See `xgrn_ext/README.md`. |
| `xgrn_ext.simdgroup_matmul_primitive` (Custom Primitive subclass, axpby-style eval_gpu) | After installing Metal Toolchain (`xcodebuild -downloadComponent MetalToolchain`), built `xgrn_ext.metallib` (12 KB) at extension build time via MLX's `mlx_build_metallib` macro. `SimdgroupMatmul : public mlx::core::Primitive` now has a working `eval_gpu`: `out.set_data(allocator::malloc(...))` + load metallib via `metal::device(s).get_library` + dispatch via `metal::get_command_encoder`. Isolated microbench: ~1.00× of MLX matmul, byte-exact parity vs raw kernel. **Integration via `--fuse-qkv-prim` at t2i-correct (default `--compile-visual-pass`)**: GRN `75.56 → 81.08 s` (+7.30 %), end-to-end +8.74 %, **decode +45.66 %**, CLIP 0.9904 → 0.9206. **Worse than `--fuse-qkv-ext`** (+3.77 % GRN). Re-bench with `--no-compile-visual-pass` to factor out compile-state effects: vs the uncompiled-only baseline (GRN 82.97 s, decode 1.85 s) the prim is +3.08 % GRN, +3.83 % e2e, **decode -24.22 %** (faster decode without compile). So the genuine kernel-integration overhead is only ~3 % — most of the +7.30 % we saw with compile on is compile-state side effects (uncompiled visual_pass also makes decode +75 % slower; same anomaly hits every qkv-fusion experiment). Net: dropping compile alone costs ~10 %, so prim+uncompiled is still +13 % e2e vs the production-default baseline. Track A1-full is **fully explored at every available framework level**; the bottleneck is the visual_pass mx.compile / Metal-pipeline interaction with custom-kernel ops, not the kernel itself. |
