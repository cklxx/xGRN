...
| `simdgroup_matmul` (Track A1-full real foundation) | New module `xgrn_mlx/simdgroup_matmul.py` with byte-exact `simdgroup_matrix<float,8,8>` and `simdgroup_matrix<bfloat,8,8>` GEMMs. 2×2 simdgroup layout (4 simdgroups per threadgroup, 16×16 output tile). Microbench at the QKV shape (M=528, K=2304, N=2304): fp32 **2.13× of MLX matmul** (2492 µs vs 1183 µs), bf16 **2.22× of MLX bf16 matmul** (2360 µs vs 1063 µs). 4×4 layout was slightly worse (2.22× fp32); threadgroup-memory tile caching was much worse (3.32×, barrier cost exceeds caching benefit). Closing the remaining 2× gap requires `simd_async_copy` for double-buffered loads and output-stationary register tiling — multi-session work. Module is **not wired into the GRN block yet**; at 2.13× of MLX it would regress total GRN time. Foundation for the real A1-full kernel. |
