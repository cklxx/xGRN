...
| `xgrn_ext.simdgroup_matmul_primitive` (Custom Primitive subclass, axpby-style eval_gpu) | After installing Metal Toolchain (`xcodebuild -downloadComponent MetalToolchain`), built `xgrn_ext.metallib` (12 KB) at extension build time via MLX's `mlx_build_metallib` macro. `SimdgroupMatmul : public mlx::core::Primitive` now has a working `eval_gpu`: `out.set_data(allocator::malloc(...))` + load metallib via `metal::device(s).get_library` + dispatch via `metal::get_command_encoder`. Isolated microbench: ~1.00× of MLX matmul, byte-exact parity vs raw kernel. **Integration via `--fuse-qkv-prim`**: t2i-correct GRN `75.56 → 81.08 s` (+7.30 %), end-to-end +8.74 %, **decode +45.66 %**, CLIP 0.9904 → 0.9206. **Worse than `--fuse-qkv-ext`** at t2i-correct scale (+3.77 % GRN) — the axpby-style per-call overhead (`set_data` allocation + `set_input_array` × 3 + `set_bytes` × 3 + `dispatch_threads`) scales poorly across 1400 qkv calls / generation. On the small debug profile (56 calls) prim was better (GRN +1.64 % vs ext's +9.5 %); the crossover means the axpby dispatch isn't optimized for high-call-count integration the way MLX's CustomKernel infrastructure is. Named-primitive status doesn't unlock additional mx.compile fusion in practice — compile's fusion rules are op-specific, not generic. Track A1-full is now **fully explored at every available framework level**; the bottleneck is genuinely the integration tax `mx.fast.metal_kernel`-style custom kernels pay inside the visual_pass compile, regardless of dispatch path. |
